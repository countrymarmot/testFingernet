...
class NormalizedCorrelationLayer(Layer):
    """
    ...
    def __init__(self, patch_size=(5, 5), dim_ordering='tf', border_mode='same', stride=(1, 1), activation=None,
                 **kwargs):
        ...
        super(NormalizedCorrelationLayer, self).__init__(**kwargs)
        ...
        for k in range(inp_shape[-1]):
            ...
            for i in range(len_xc_1):
                ...
                block.append(K.reshape(K.batch_dot(xc_2_aggregate[:, sl1, :],
                                                   xc_1_aggregate[:, :, i]),
                                       (-1, 1, 1, inp_shape[1] * self.kernel_size[0])))
    ...
    def get_config(self):
        ...
        base_config = super(NormalizedCorrelationLayer, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))


def normalized_x_corr_model(im_input_shape):
    ...
    model1 = model(b)
    model2 = model(a)
    normalized_layer = NormalizedCorrelationLayer(stride=(1, 1), patch_size=(5, 5))([model1, model2])
...
def norm_model(input_size=(8, 8, 2)):
    a = Input(input_size)
    b = Input(input_size)
    output = NormalizedCorrelationLayer(stride=(1, 1), patch_size=(5, 5))([a, b])
